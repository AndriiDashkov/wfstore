...
public class WsNewPrihodDialog extends JDialog {
	...
	protected WsPrihodPartEditTable m_table = null;
	...
	public WsNewPrihodDialog(JFrame jfrm, WsPrihodData dt, String nameFrame) {
		...
		m_table = new WsPrihodPartEditTable(this);
		...
		Dimension screenDim = WsGuiTools.getScreenSize();
		
		setMinimumSize(new Dimension(screenDim.width - 100, screenDim.height - 50 ));
	...
	public int getContractId() {
		...
		return m_contractsCombo.getCurrentSQLId();
	}
	...
	private void insertData() {
		...
		if(vec.isEmpty()) {
			...
			WsUtils.showMessageDialogLong3(getMessagesStrs("emptyCatalogSelectionSuccess0"),
					 getMessagesStrs("emptyCatalogSelectionSuccess1"),
					 getMessagesStrs("emptyCatalogSelectionSuccess2"));
			
			return;
			
		}
		
		insertCostFromContract(vec, m_contractsCombo.getCurrentSQLId());
	...
	private void newPartOpenDialog() {
		...
		WsNewPartDialog dialog = new WsNewPartDialog(WsUtils.get().getMainWindow(), null, 
	...
	public void onOK(ActionEvent e) {
		...
		if(m_data.id == -1) {
			...
			String operationMessage = "<html>" + getMessagesStrs("updatePrihodFaultWoringDate0") +
					"<br/>" + getMessagesStrs("updatePrihodFaultWoringDate1") + "</html>";
	...
	public void insertCostFromContract(Vector<WsPartType> vec, int id_contract) {
	...
	private void setCustomFont() {
		...
		m_kod_table.setRowHeight(m_kod_table.getRowHeight() + margin_table);
		
		WsGuiTools.changeFont(m_table, f);
		
		m_table.getTableHeader().setFont(f);
		
		m_kod_table.getTableHeader().setFont(f);
