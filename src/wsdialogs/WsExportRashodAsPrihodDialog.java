...
public class  WsExportRashodAsPrihodDialog extends JDialog {
	...
	public  WsExportRashodAsPrihodDialog(JFrame jf, String caption,  Vector<WsRashodData> dt) {
		...
		if(!dt.isEmpty()) {
			...
			m_initial_date = new java.sql.Date(dt.elementAt(0).date.getTime());
	...
	private void createGUI() {
		...
		m_title = BorderFactory.createTitledBorder(getGuiStrs("multiSelectionBorderTitle"));
		
		panel_2.setBorder(m_title);
	...
	private void exportDataMulti() {
		...
		try {
			...
			if(a_vec.isEmpty()) {
				...
				WsUtils.showMessageDialog(getMessagesStrs("noAgentsExportFailed"));
			...
			for(int i = 0; i < out_vec.size(); ++i) {
			 ...
			 if(f != -1 ) {
				 ...
				 if(notFindKods.isEmpty()) {
					 ...
					 WsUtils.showMessageDialog(getMessagesStrs("ExportNaklPrihSuccess"));
				 }
				 else {
					
					 WsUtils.showMessageDialog(getMessagesStrs("ExportNaklPrihSuccessKods") + notFindKods);
				...
				WsUtils.showMessageDialog(getMessagesStrs("ExportNaklPrihFailed"));
