...
public class WsLog {
	...
	private static WsLog instance = null;
	...
	private WsLog() 
	{	
		filePath = WsUtils.get().getLogFullPath();
	};
	
	public static WsLog get() {
	...
	private static void logFileSizeControl()
	{
		...
		double megabytes = WsUtils.get().getSettings().getMaxLogFileLength();
		...
		m_logIsEnabled = WsUtils.get().getSettings().isLogEnabled();
	 ...
	 * @param exp - exception which about info must be shown (can be null)
			...
			boolean infoFlag, boolean printExcTrace ) 
	{
		...
		if ( infoFlag ) {
			...
			WsUtils.get().getMainLabel().setText(s);
		}
