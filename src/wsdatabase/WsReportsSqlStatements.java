...
public static Vector<WsSkladMoveDataRow> getRashodMovement46(Connection conn, Date start_d, 
	...
	public static Vector<WsSkladMoveDataColumn> getMovePartsForAllContracts( Date start_date,
	      ...
	      // sb.append("SUM(CASE WHEN invoices.date < ? THEN invoice_parts.quantity*invoice_parts.costnds ELSE 0.0 END) as in_r_sum,  ");
	       ...
	       sb.append("INNER JOIN contracts ON invoices.id_contract =  contracts.id ");
	       sb.append("WHERE invoices.date <= ? ");
	       sb.append("GROUP BY part_types.kod, invoices.id_contract ORDER BY part_types.kod) AS t1 ");
	       sb.append("LEFT JOIN ");
	       sb.append("(SELECT part_types.kod as kd,  invoices.id_contract as idc,");
	       sb.append("SUM(CASE WHEN sale_invoices.date < ? THEN sale_parts.quantity ELSE 0.0 END) as out_r,");
	       sb.append("SUM(CASE WHEN sale_invoices.date < ? THEN sale_parts.quantity*(invoice_parts.cost + invoice_parts.nds) ELSE 0.0 END) as out_r_sum,  ");
	       ...
	       sb.append("WHERE sale_invoices.date <= ? ");
	       sb.append("GROUP BY part_types.kod, invoices.id_contract ORDER BY part_types.kod, invoice_parts.id) AS t2 ON t2.idc = t1.idc AND t1.kd = t2.kd ");
	       sb.append("WHERE init_rest > ? OR t1.in_q > ? ORDER BY t1.idc;");
		   ...
		   try {
	        	...
	        	WsSkladMoveDataColumn data_contr_sum = new WsSkladMoveDataColumn();
	        	
	 	        Vector<WsSkladMoveDataColumn> vec_with_sums = new Vector<WsSkladMoveDataColumn>();
	    		...
	    		for(int i = 0; i <  (vec.size() - 1); ++i) {
	    			...
	    			data_contr_sum.q_array[0].initial_rest_sum += d.q_array[0].initial_rest_sum;
	    			
	    			data_contr_sum.q_array[0].in_quantity_sum += d.q_array[0].in_quantity_sum;
	    			
	    			data_contr_sum.q_array[0].out_quantity_sum += d.q_array[0].out_quantity_sum;
	    			
	    			vec_with_sums.add(d);
	    				
	    			WsSkladMoveDataColumn d_next =  vec.elementAt(i + 1);
	    				
	    			if(d_next.id_contract != d.id_contract) {
	    					
    					data_contr_sum.q_array[0].rest_sum = data_contr_sum.q_array[0].initial_rest_sum + data_contr_sum.q_array[0].in_quantity_sum - data_contr_sum.q_array[0].out_quantity_sum;
    		    		
    					data_contr_sum.id_contract = -1;
    					
    					data_contr_sum.name = WsUtils.getGuiStrs("vsegoPoDog") + " : " + d.contract_name;
    					
    					vec_with_sums.add(data_contr_sum);
    					
    					data_contr_sum = new WsSkladMoveDataColumn();
	    		...
	    		WsSkladMoveDataColumn d =  vec.elementAt((vec.size() - 1));	
			...
			for(int i = 1; i < agents_vec.size(); ++i) {
				...
				for(WsSkladMoveDataColumn d: data) {
					...
					if(map.containsKey(d.kod)) {
						...
						WsSkladMoveDataColumn d_ = map.get(d.kod);
