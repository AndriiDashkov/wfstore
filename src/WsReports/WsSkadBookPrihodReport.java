 ...
 *
 */
public class WsSkadBookPrihodReport extends WSReportViewer {

	Vector<WsSkladMoveDataRow> m_vec_all_parts = null;

	public WsSkadBookPrihodReport(JFrame f, String nameFrame) {
		super(f, nameFrame);
		...
		m_genButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
            	 ...
            	 if(m_html_pages.isEmpty()) { return; }
		...
		for(int i = 0; i < vec_parts.size(); ++i) {
			...
			for(int k = 0; k < d_row.pages_number; ++k) {
				...
				for(int j = 0; j < row_vec.size(); ++j) {
					 ...
					 vec_itogo.set(j, vec_itogo.elementAt(j) + dc.in_quantity);
	...
	public Vector<String> generateReport() {
		...
		Vector<WsSkladMoveDataRow> vec_all_parts = WsReportsSqlStatements.getSkladPrihodMovement(m_date.getSqlStartDate(), m_date.getSqlEndDate());;

		int columns_number = syncColumns(vec_all_parts);
		
		//divide into pages
		int columns_per_page = 10;
	...
	public String getPrintHtml(Vector<WsSkladMoveDataRow> vec_all_parts, Vector<Double> vec_itogo, int page_number) {
		...
		sHeader_b.append( getGuiStrs("reportBookNameGoodColumn"));
		
		sHeader_b.append( "</font></td>");
		
		StringBuilder sHeader3_b =  new StringBuilder();
						
		sHeader_b.append("<tr><td colspan='2' style='border-left: 1px solid;border-top: 1px solid ; ext-align: center;'><font size =4> ");
		
		sHeader_b.append(getGuiStrs("reportBookNameColumn") );
		...
		for(int i = 0; i < vec_all_parts.size(); ++i) {
			...
			row_s_b.append(WsUtils.dateToString(d_row.date_doc, "dd.MM.yy" ));
		...
		hS_b.append( getGuiStrs("bookSkladPrihodReportName") );
		...
		hS_b.append( "hS_b.append(<br><table style='width:60%;'  BORDER=0 CELLPADDING=0 CELLSPACING=0>"
		...
		try {
	    	...
	    	for (WsSkladMoveDataRow dt: vec_all_parts) {
                ...
                if(dt.indexData == 0) {
                	...
                	cell02.setCellValue(getGuiStrs("naklReportName2") + " "+ dt.nakl_number + " " + getGuiStrs("vidNaklName2") +" " + WsUtils.dateSqlToString(dt.date_doc, "dd.MM.yy"));
                }
	            ...
	            for (int j = 0; j < vec_columns.size(); j++) {
	                ...
	                if(col_data.in_quantity < 0.00001) {
	                	 ...
	                	 cell1.setCellValue(col_data.in_quantity);
	                }
	...
	private void createExcelHeader( XSSFSheet sheet, Vector<WsSkladMoveDataRow> vec_all_parts, XSSFCreationHelper creationHelper) {
		   ...
		   for(int i = 0; i < vec_columns.size(); ++i) {
			   ...
			   createCell(rowHeader2, cell_index++, getGuiStrs("quantityNameReportColumn"), creationHelper);
