 ...
 * 
 */
public class WsPorivSkladWithFutureRashod  extends WSReportViewer {
	...
	WsFileChooser[] m_fileChoosers = {
			new WsFileChooser(getGuiStrs("fileChooserNameMr1")),
	...
	public WsPorivSkladWithFutureRashod(JFrame f, String nameFrame) {
		super(f, nameFrame);
	...
	public String getPrintHtml(Vector<WsSkladMoveDataColumn> vec_all, int start, int end, int page_number) {
		...
		StringBuilder sHeader_b =  new StringBuilder(500);
		...
		sHeader_b.append(getGuiStrs("pribZalishotPlusReportName") );
		...
		sHeader_b.append(getGuiStrs("planRashodReportName2") );
		...
		sHeader_b.append(getGuiStrs("faktSkladName")); 
		...
		sHeader_b.append(getGuiStrs("planRashodReportName3")); 
		...
		StringBuilder row_s_b = new StringBuilder(500);
		...
		for(int i = start; i <= end; ++i) {
			 ...
			 double min = d.q_array[0].initial_rest < d.q_array[3].out_quantity ? d.q_array[0].initial_rest :  d.q_array[3].out_quantity;
			 
			 double diff_available_to_move_plan_rashod = (min + d.q_array[0].in_quantity) - (d.q_array[1].out_quantity + d.q_array[2].out_quantity) ;
			 ...
			 row_s_b.append(d.q_array[0].initial_rest);
			 ...
			 row_s_b.append(d.q_array[2].out_quantity);
			 ...
			 row_s_b.append(WsUtils.getDF(d.q_array[0].in_quantity + d.q_array[0].initial_rest - d.q_array[1].out_quantity - d.q_array[2].out_quantity) );
			 ...
			 row_s_b.append(WsUtils.getDF(d.q_array[3].out_quantity));
			 ...
			 row_s_b.append(WsUtils.getDF(d.q_array[3].out_quantity - d.q_array[1].out_quantity - d.q_array[2].out_quantity));
			 ...
			 row_s_b.append(WsUtils.getDF(diff_available_to_move_plan_rashod) );
			 
			 row_s_b.append("&nbsp;</font></td></tr>");
		
		}
		
		StringBuilder hS_b = new StringBuilder(1000);
		...
		hS_b.append(getGuiStrs("porivSkaldFutureRashodCompareReportName") );
	...
	public void exportToExcelFile(Vector<WsSkladMoveDataColumn> vec_all_parts) {
		...
		try {
	    	...
	    	for (WsSkladMoveDataColumn dt: vec_all_parts) {
	                ...
	                cell05.setCellValue(dt.q_array[0].in_quantity + dt.q_array[0].initial_rest);
	                
	                XSSFCell cell06 = row.createCell(cell_index++);
	                
	                cell06.setCellValue(dt.q_array[1].out_quantity + dt.q_array[2].out_quantity);
	                
	                XSSFCell cell07 = row.createCell(cell_index++);
	                
	                cell07.setCellValue(dt.q_array[0].in_quantity + dt.q_array[0].initial_rest - dt.q_array[1].out_quantity - dt.q_array[2].out_quantity);
	                
	                XSSFCell cell08 = row.createCell(cell_index++);
	                
	                cell08.setCellValue(dt.q_array[3].out_quantity);
	                
	                XSSFCell cell09 = row.createCell(cell_index++);
	          
	                String rowNum = String.valueOf(row_index);
	                
	                String f = "MIN(H"+ rowNum + ",C" + rowNum +")+D" + rowNum;
	                
	                cell09.setCellFormula(f);
	              
	                XSSFCell cell10 = row.createCell(cell_index++);
	  	                       
	                f = "I" + rowNum + "-F" + rowNum;
	                
	                cell10.setCellFormula(f);
	...
	private void createExcelHeader( XSSFSheet sheet, XSSFCreationHelper creationHelper) {
		   ...
		   createCell(rowHeader0, 2, getGuiStrs("zalNaNachaloReportName") + ":", creationHelper);
		   
		   createCell(rowHeader0, 3, getGuiStrs("pribuloReportName") + ":", creationHelper);
		   
		   createCell(rowHeader0, 4, getGuiStrs("prihodPartsColumnRestName") + " + " + getGuiStrs("pribuloReportName") + ":", creationHelper);
		   
		   createCell(rowHeader0, 5, getGuiStrs("planRashodSpisReportName") + ":", creationHelper);
		   
		   createCell(rowHeader0, 6, getGuiStrs("rizZalPrib2ReportName") + ":", creationHelper);
		   
		   createCell(rowHeader0, 7, getGuiStrs("realZalishokReportName"), creationHelper);
		   
		   createCell(rowHeader0, 8, getGuiStrs("mozlivoSpisReporName"), creationHelper);
		   
		   createCell(rowHeader0, 9, getGuiStrs("diff3ReportName"), creationHelper);
