 ...
 *
 */
public class WFNewPartDialog extends JDialog  {
	...
	WsPartType m_data = new WsPartType();
	
	public WFNewPartDialog(JFrame jfrm, WsAgentData dt, String nameFrame) {
	...
	public void onOK(ActionEvent e) {
		...
		m_data.kod = WsUtils.getUnsignedIntegerFromString(m_kod.getText(), getMessagesStrs("wrongStringforIntegerKod"));
		
		if(m_data.kod < 0) { return; }
		
		m_data.name = m_name.getText();
		
		m_data.info = m_info.getText();
		
		m_data.id = WsUtilSqlStatements.insertPartType(m_data);
 	   
        WsEventInt event = new WsEventEnable(WsEventEnable.TYPE.NEW_PART_TYPE_CREATED);
	...
	private JPanel createGUI() {
		...
		m_name_label = new JLabel(getGuiStrs("partNameNewDialogLabel"));
		
		m_kod_label = new JLabel(getGuiStrs("kodNewPartDialogLabel"));
		
		m_info_label = new JLabel(getGuiStrs("infoPartNewDialogLabel"));
		...
		panel_1.add(m_kod_label);    panel_1.add(m_kod);
	...
	private void setAllListeners() {
		...
		m_kod.addFocusListener(fListener);
	...
	public void verifyInfo() {
		...
		if ( WsTokenizer.isValidate(m_name.getText()) == false ||
				WsTokenizer.isValidate( m_info.getText()) == false ||  (WsUtils.getUnsignedIntegerFromString(m_kod.getText(), null) < 0 )) {
