 ...
 *
 */
public class WsExcelImportPrihodDialog extends JDialog {
	...
	public WsExcelImportPrihodDialog(JFrame jf, String caption) {
	...
	private void createGUI() {
		...
		m_panel_ind = new WsIndicesImportPanel(this, TYPE.PRIHODNAKL);
	...
	public void importData() {
		...
		if(excel_file_name == null || excel_file_name.isEmpty()) {
			...
			WsUtils.showMessageDialog(getMessagesStrs("excelFileNotFoundCaption"));
		...
		if(data_import == null) {
			...
			WsUtils.showMessageDialog(getMessagesStrs("wrongExcelFileFormatInfoCaption"));
		...
		for(int i =0; i <  data_import.m_data.size(); ++i) {
			...
			WsRowData d = data_import.m_data.elementAt(i);
			...
			if(pt == null) {
				   ...
				   WsUtils.showMessageDialog(getMessagesStrs("cantDetectPartTypeCaption") + " " + String.valueOf(d.kod) );
		...
		if( warnMessage != null) {
			   ...
			   WsUtils.showMessageDialog( warnMessage );
		}
		...
		if(inserted_id != -1) {
			...
			WsUtils.showMessageDialog(getMessagesStrs("messageProhNaklSuccess"));
		}
		else {
			...
			WsUtils.showMessageDialog(getMessagesStrs("messageProhNaklFailed"));
		}
